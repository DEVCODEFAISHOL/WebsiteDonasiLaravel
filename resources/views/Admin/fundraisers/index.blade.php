        ...
        <div class="list-fundraisers py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                  {{-- Fetching fundraiser data from FundraiserController --}}
                    ...
                    <div class="item-card flex flex-row justify-between items-center">
                        ...
                        <div class="hidden md:flex flex-col">
                            <p class="text-slate-600 text-md">Date</p>
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $fundraiser->created_at }}</h3>
                        </div>
                        <span class="w-fit text-sm font-bold py-2 px-3 rounded-r-md bg-green-500 text-white">
                            ACTIVE
                        </span>

                        <span class="w-fit text-sm font-bold py-2 px-3 rounded-r-md bg-orange-500 text-white">
                            PENDING
                        </span>
                        <div class="hidden md:flex flex-row items-center gap-x-3">
                            <form action="#" method="POST">
                                @csrf
                                @method('PUT')
                                <button type="submit" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-r-md">
                                    Approve
                                </button>
                            </form>
                        </div>
                        <div class="hidden md:flex flex-row items-center gap-x-3">
                            <form action="#" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="font-bold py-4 px-6 bg-red-700 text-white rounded-r-md">
                                    Delete
                                </button>
                            </form>
                        </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col ">
                    <div class="item-card flex flex-col justify-between items-center gap-y-5">
                        ...
                        @elseif($fundraiserStatus == 'Active')
                            <a href="#" class="font-bold py-4 px-2 bg-indigo-50 text white rounded-r-md"> 
                                Create A Fundraing
                            </a>
                            @else
